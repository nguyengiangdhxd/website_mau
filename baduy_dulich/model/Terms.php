...
class Terms extends \TermsBase {
    ...
    public function getSiteRoot(){
        $query = \Terms::select();
        $query->andWhere(" `lft` <= {$this->getLft()} ");
        $query->andWhere(" `rgt` >= {$this->getRgt()} ");
        $query->andWhere(" `lvl` <= {$this->getLvl()} ");
        $query->andWhere(" `status` = 'ACTIVE' ");
        $query->andWhere(" `scope` = 'category' ");
        $query->andWhere(" `parent_id` != 0 ");
        $query->andWhere(" `id` != 27 ");//Bỏ qua danh mục gốc
